...
- **Eligibility and self-invalidation fall out of the signal, not a route check** — but via *reproducibility*, not the confidence ratio. Implementation corrected an earlier claim here: `peak_ratio` is `max/mean` over a ~118k-lag search range, so by extreme-value statistics a single source with no second copy still scores around 6 against a 4.0 threshold. It filters, it does not decide. What separates a real second copy from reverb or noise is that a genuine lag **reproduces across windows** while a spurious peak lands somewhere new each time — hence the ≥3-window agreement rule. Headphones and a silent room both fail that test by construction; `acousticallyReachable` just skips them earlier.
